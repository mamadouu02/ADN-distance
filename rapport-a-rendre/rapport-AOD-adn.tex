...
L'algorithme qui se comporte le mieux est CA qui est meilleur en temps, énergie. Plusieurs raison peuvent justifier cela : L'itératif est plus lent due à son nombre de défault de cache très grand devant celui de CA et pour CO qui a les mêmes défault de cache c'est peut-être le nombre d'appel récursive.
...
le plus performant pour la commande ci dessus (test 5); les ressources pour l'exécution seraient environ: on effectue une régression polynomiale avec les valeurs précédentes avec $N=20000000$ et $M=20000000$ ($R^2=0.99$)
\begin{itemize}
